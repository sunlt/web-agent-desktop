...
- [x] ChatUI 新增“执行器工作目录文件管理器”：按 `sessionId` 浏览 `/workspace`、支持预览/编辑/上传/下载/重命名/删除。
- [x] ChatUI 新增 `TTY` 面板：按 `sessionId` 执行命令并展示 `stdout/stderr/exitCode`（超时仅提示，不自动完成任何流程）。
...
- `control-plane/src/services/reconcile-metrics-prometheus.ts`
