...
CMD ["/bin/bash", "-c", "sed -i 's/^#\\?PermitRootLogin.*/PermitRootLogin yes/' /etc/ssh/sshd_config && sed -i 's/^#\\?PasswordAuthentication.*/PasswordAuthentication no/' /etc/ssh/sshd_config && sed -i 's/^#\\?PubkeyAuthentication.*/PubkeyAuthentication yes/' /etc/ssh/sshd_config && mkdir -p /var/run/sshd && ssh-keygen -A && /usr/sbin/sshd && exec opencode web --port 4096 --hostname 0.0.0.0"]
